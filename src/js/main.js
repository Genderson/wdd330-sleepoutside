...
for (let i = 1; i < 5; i++) {
    let productHref = document.querySelector('.product-card:nth-of-type('+i+') a').href;
    let productId = getProductKey(productHref);
    let product = await findProductById(productId);
    let productCardPrice = document.querySelector('.product-card:nth-of-type('+i+') p');
    ...
    let productDiscountPercentage = `${(100 * (1 - product.FinalPrice/product.ListPrice)).toFixed(0)}%`;
...
function getProductKey(productHref) {
    let productKey;
    return productKey = productHref.slice(productHref.indexOf("product=")+8, productHref.length);
}