document.addEventListener("DOMContentLoaded", () => {
  ...
  function fetchComments() {
    const comments =
      JSON.parse(localStorage.getItem(`comments_${productId}`)) || []; // Get the comments from local storage or initialize an empty array
    commentsList.innerHTML = comments
      .map((comment) => `<li>${comment}</li>`)
      .join(""); // Render the comments as list items and update the comments list element
  }
